...
class Plugin():
  ...
  regexes = {
    ...
    "phone_with_ext": re.compile('((?:(?:\+?1\s*(?:[.-]\s*)?)?(?:\(\s*(?:[2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9])\s*\)|(?:[2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9]))\s*(?:[.-]\s*)?)?(?:[2-9]1[02-9]|[2-9][02-9]1|[2-9][02-9]{2})\s*(?:[.-]\s*)?(?:[0-9]{4})(?:\s*(?:#|x\.?|ext\.?|extension)\s*(?:\d+)?))', re.IGNORECASE),
  ...
  # Uncomment the filter_event method if you want to filter inbound events using the regexes above
  # def filter_event(self, event):
  #   print(f"scrubber plugin event before: {json.dumps(event)}")
  #   r = self.scrub(event)
  #   print(f"scrubber plugin event after: {json.dumps(r)}")
  #   return r

  # Uncomment the filter_webhook method if you want to filter outbound webhooks using the regexes above
  # def filter_webhook(self, webhook):
  #   print(f"scrubber plugin webhook before: {json.dumps(webhook)}")
  #   r = self.scrub(webhook)
  #   print(f"scrubber plugin webhook after: {json.dumps(r)}")
  #   return r
